...
return function(loader)
  ...
  Map = loader.class("Map")

  local Tile = loader.class("Tile")

  local tile_id_generator = loader.load("uti").idGenerator("tile")
  ...
  function Map.static.load(path)
    ...
    local map_data = AssetLoader.loadRaw(path)
    local tileset = AssetLoader.load("spritesheet", map_data.tileset)
  ...
  function Map:pathTo(from, to, out, walkable_only)
    local path = astar.getPath(self:getTile(from), self:getTile(to), self.tiles,
      function(tile)
        local ret = {}
        for _,t in pairs(tile.neighbors.all) do
          if walkable_only == false or t.walkable or t.coordinates == from then
            table.insert(ret, t)
          end
        end
        return ret
      ...
      function(a, b, n)
        return 1 + math.abs(a.height - b.height) * 10
      end,
      function(a, b)
        return
          a.coordinates:dist2(b.coordinates)
