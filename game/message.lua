...
function Message:draw()
  ...
  sg.print(self.title[locale], self.margin, self.margin)

  self.font.description:apply()
  sg.printf(self.message[locale], self.margin, self.margin * 2 + self.title_size, self.w)

  sg.setColor(255, 0, 0)
  local dx = math.random(self.continue.jitter)
  local dy = math.random(self.continue.jitter)
  sg.printf(self.continue[locale], 60 + dx, self.h - self.description_size - 20 + dy, self.w - 120, "center")
